...
function aLeftForm1Sub(){
	...
	$.post("./index.php?a=logincheck",{action:"login",username:un,password:pw},function(login_jg){
		var data = eval('('+login_jg+')');
		if(data['error'].indexOf("2")==0){
			alert("账号错误请重新输入!");
			$("#rmNum").val(''); //清空验证码
			$("#password").val(''); //清空验证码
			$("#username").select();
			...
			window.location.href='./index.php?a=login_info&info=1';
			return;
		}
