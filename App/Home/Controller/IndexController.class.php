...
class IndexController extends CommonController {
    ...
    public function ajax_odd() {
        ...
        if($user['peilv'] == '0'){
            ...
            $arrs_hui = $this->haomas($datas, $data[0], $data[2], $markets, $bets, $data[3], $types, $prohibit,$usermoney,$molls);
            //会员下级赔率
            $molls['userloss'] = $user['loss'];
            $molls['user'] = $arrs_hui[0];
            $molls['peilv'] = $user['peilv'];
        }
        $datas['m_loss']=$user['loss'];
        $arrs = $this->haomas($datas, $data[0], $data[2], $markets, $bets, $data[3], $types, $prohibit,$usermoney,$molls);
        if ($arrs['code'] == 400) {
            echo -5;
            exit;
        }
    ...
    //公共方法得到号码的赔率
    private function haomas($datas, $types, $num, $data2, $bets, $status8, $types1, $prohibits,$usermoney,$molls) { //股东赔率/单注上限
        ...
        if ($sendqz == 1 && $data[3] == '4定') { //得到全转号码
           ...
           // }
            ...
            foreach ($haomaArrs1 as $k => $v) {
                ...
                $arrs = $this->haomas($datas, $data[0], $v, $markets, $bets, $data[3], $types, $prohibits,$money,$molss);
                //总代理赔率
                $datas['m_loss']=$user_zd['m_loss'];
                $arrs_zd = $this->haomas($datas, $data[0], $v, $markets, $bets, $data[3], $types, $prohibit,$money,$molss);

                //代理赔率
                $datas['m_loss']=$user_dl['m_loss'];
                $arrs_dl = $this->haomas($datas, $data[0], $v, $markets, $bets, $data[3], $types, $prohibit,$money,$molss);
                //会员赔率
                $datas['m_loss']=$user['m_loss'];
                $arrs_hui = $this->haomas($datas, $data[0], $v, $markets, $bets, $data[3], $types, $prohibit,$money,$molss);

                //会员下级赔率
                $molss['userloss'] = $user['loss'];
                $molss['user'] = $arrs_hui[0];
                $datas['m_loss']=$user['loss'];
                $arrs_huiyuan = $this->haomas($datas, $data[0], $v, $markets, $bets, $data[3], $types, $prohibit,$money,$molss);
                if($arrs[0] == null || $arrs_zd[0] == null || $arrs_dl[0] == null || $arrs_hui[0] == null || $arrs_huiyuan[0] == null){
                    echo -6;
                    exit;
                }
                if ($arrs['code'] != 400) {
                    $moneys1+= $money;
                    $data1['odds'] = $arrs_huiyuan[0];
                    $data1['odds_gd'] = $arrs[0];
                    $data1['odds_zd'] = $arrs_zd[0];
                    $data1['odds_dl'] = $arrs_dl[0];
                    $data1['odds_hy'] = $arrs_hui[0];
                    $data1['bianse'] = $arrs_huiyuan[6];
                    $data1['win'] = $money * $arrs[4];
                    $data1['topwin'] = $arrs[4] * $data1['topmoney']; //拦货回水
                    if($arrs[0] != 0 || $arrs_zd[0] != 0 || $arrs_dl[0] != 0 || $arrs_hui[0] != 0 || $arrs_huiyuan[0] != 0){
                        $data1['js'] = 0;
                    }else{
                        echo -6;
                        exit;
            ...
            $top_zd['au_id']=$user_dl['top_uid'];
            $user_zd = M('admin')->where($top_zd)->find();
            $datas['peilv'] = $user['peilv'];
            $molss = $datas;
            $arrs = $this->haomas($datas, $data[0], $data[2], $markets, $bets, $data[3], $types, $prohibits,$money,$molss);
            //总代理赔率
            $datas['m_loss']=$user_zd['m_loss'];
            $arrs_zd = $this->haomas($datas, $data[0], $data[2], $markets, $bets, $data[3], $types, $prohibit,$money,$molss);
            //代理赔率
            $datas['m_loss']=$user_dl['m_loss'];
            $arrs_dl = $this->haomas($datas, $data[0], $data[2], $markets, $bets, $data[3], $types, $prohibit,$money,$molss);
            //会员赔率
            $datas['m_loss']=$user['m_loss'];
            $arrs_hui = $this->haomas($datas, $data[0], $data[2], $markets, $bets, $data[3], $types, $prohibit,$money,$molss);
            //会员下级赔率
            $molss['userloss'] = $user['loss'];
            $molss['user'] = $arrs_hui[0];
            $datas['m_loss']=$user['loss'];
            $arrs_huiyuan = $this->haomas($datas, $data[0], $data[2], $markets, $bets, $data[3], $types, $prohibit,$money,$molss);
            if($arrs[0] == null || $arrs_zd[0] == null || $arrs_dl[0] == null || $arrs_hui[0] == null || $arrs_huiyuan[0] == null){
                echo -6;
                exit;
            }
...
//            }
            ...
            $bet['mingxi_3'] = $data[4];
            $bet['types'] = $data[1];
            $bet['odds'] = $arrs_huiyuan[0];
            $bet['odds_gd'] = $arrs[0];
            $bet['odds_zd'] = $arrs_zd[0];
            $bet['odds_dl'] = $arrs_dl[0];
            $bet['odds_hy'] = $arrs_hui[0];
            $bet['loss'] = $arrs[4];
    ...
    public function xiazhu_erding() {
        ...
        if (strlen($haoma) == 4) {
            ...
            $data1['money'] = $money;
            $odds_gd = get_odd3($haoma, $weishu, $bet['mingxi_3'], $data, $data1, $qishu, $prohibits, $markets1, $bets);
            ...
            if($odds_gd[0] == null || $odds_zd[0] == null || $odds_dl[0] == null || $odds_hy[0] == null || $odds[0] == null){
                echo -6;
                exit;
            }
//             var_dump($odds);exit
            if ($odds['code'] != 400 && $money <= $odds[2] && $money >= $odds[3]) {
                if ($money > $moneys1) { //下注积分大于
                    ...
                    $bet['topwin'] = $odds[1] * $toparr[0];
                    $moneys+= $money;
                    if($odds_gd[0] == 0 || $odds_zd[0] == 0 || $odds_dl[0] == 0 || $odds_hy[0] == 0 || $odds[0] == 0){
                        echo -6;
                        exit;
                    }
                }
        ...
        if (strlen($haoma) > 11) {
            ...
            for ($a = 0;$a < count($strs);$a++) {
                ...
                if ($strs1[0] && $strs1[1]) {
                    ...
                    $bet['money'] = $money;
                    //分别查赔率
                    $odds_gd = get_odd3($haoma1, $weishu, $bet['mingxi_3'], $data, $data1, $qishu, $prohibits, $markets1, $bets);
                    $data1['m_loss']=$user_zd['m_loss'];
        ...
        //$numbers=M('number');
         foreach ($data as $k => $v) {
            ...
            $bet[$k]['mingxi_2'] = $v;
             ...
             $top_zd['au_id']=$user_dl['top_uid'];
             $user_zd = M('admin')->where($top_zd)->find();
             $arrs_gd = $this->haomas2($datas, $weishu, $v, $mingxi_1, $prohibits, $bets1, $markets1, $numbers,$money,$molss);
             $datas['m_loss']=$user_zd['m_loss'];
             ...
             if($arrs[0] == null || $arrs_zd[0] == null || $arrs_dl[0] == null || $arrs_hy[0] == null || $arrs_gd[0] == null){
                 echo -6;
                 exit;
             }
             if($arrs[0] == 0 || $arrs_zd[0] == 0 || $arrs_dl[0] == 0 || $arrs_hy[0] == 0 || $arrs_gd[0] == 0){
                 echo -6;
                 exit;
             }
            $bet[$k]['uid'] = $uid;
...
//            var_dump($data1);exit;
                ...
                foreach ($datas as $k1 => $v1) {
                    foreach ($data as $k2 => $v2) {
                        if ($k1 == $v2['qishu']) {
                            ...
                            $data3['win'] = number_format($zonghuishui,2,'.','');
                        }
                    }
                    if ($data3) {
                        if ($v1 == 3) {
    ...
    public function user1() {
        ...
        $peilv = I('post.isfpfrankhotzhuan');
        $uid = session('userid');
        $where['uid'] = $uid;
