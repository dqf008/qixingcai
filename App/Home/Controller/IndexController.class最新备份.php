...
class IndexController extends CommonController {
    ...
    public function appindexajax(){
      ...
      //$data['moneys']=user();//用户总积分
        ...
        $dd=$bets->field('id,did,money,addtime,mingxi_2,mingxi_3,odds,index_id')->where($where)->order('id desc')->select();
        //dump($dd);exit;
        // $where1['uid']=$userid; 
        // $where1['qishu']=$qishu;
        // $where1['js']=0;
    ...
    public function xiazhukuang(){
        ...
        $data['data']=M('bet')->field('js,id,money,did,mingxi_2,mingxi_3,odds')->where($where)->order('id desc')->limit(10)->select();
        if(!empty($data['data'])){
            $data['code']=200;
        }else{
            $data['code']=400;
...
public function xiazhu(){
          ...
          exit;
           ...
           //会员赔率和股东原始赔率
            ...
            if($sendqz==1 && $data[3]=='4定'){//得到全转号码
             ...
             //  $arrs=$this->haomas1($datas,$loss,$data[0],$haomaArrs1[0],$markets,$bets,$data[3],$types,$prohibits);
             // dump($arrs);exit; 
    ...
    //下注代理拦货数据汇总
    private function uprocess($data,$typs,$money){
      $arr[0]=0;
      $arr[1]=0;
      $arr[2]=0;
      if($data){
 ...
 //下注_二定模式一
  ...
  public function xiazhu_kuaixuan(){
    ...
    public function getQishu(){
        ...
        $data=M('bet')->field('win,qishu,addtime,money,odds,status')->where($where2)->order('id desc')->select();
