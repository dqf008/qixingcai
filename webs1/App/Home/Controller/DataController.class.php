...
class DataController extends CommonController {
   ...
   // public function download1(){
    ...
    public function ureports(){
        ...
        $qishus=M('opentime')->field('qishu,m_status')->order('id desc')->find();
        ...
        set_time_limit(0);
        $datas=M('bet')->field('uid,win,money as moneys,js,qishu,odds,mingxi_1,status,topmoney,topwin')->where($where1)->select();
        //dump($datas);
		//dump($utype);
        if(!empty($datas) && !empty($data[1])){
            ...
            if($utype=='agency'){
              ...
              //dump(users($datas1[1]['au_id']));exit;
            foreach($datas1 as $k1=>$v1){
                if($udata){
                 ...
                 if(!empty($uids[0])){
                    ...
                    foreach($datas as $k2=>$v2){
                        ...
                        if(in_array($v2['uid'],$uidarr)){
                            ...
                            //$arr['yingkui']=$arr['moneys']+$arr['win']-$arr['money'];
                        }  
                    }
                    //dump($arr);
                    if($arr){//会员的销售额-佣金-中奖金额=会员的盈亏
                        ...
                        $data2['huishui2']+=$arr['huishui2'];//总代回水
                        $data2['sum']+=$arr['sum'];//下注笔数
                        $data2['money']+=$arr['money'];//金额
                        $data2['win1']+=$arr['win'];//回水
                        $data2['moneys1']+=$arr['moneys'];//中奖汇总
