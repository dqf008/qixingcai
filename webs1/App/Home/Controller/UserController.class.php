...
class UserController extends CommonController {
    ...
    public function addUser(){
        ...
        if($utype=='admin'){
          $where1['au_type']='partner';
           ...
           //$status1=true;
        }elseif($utype!='admin' || $utype!='partner'){
    ...
    public function saveUser(){
      //超管(admin),股东(partner),总代理(agencys),代理(agency)
        ...
        if($utype=='agencys' && !empty($user1) && empty($uid)){
          ...
          $data['au_money']=$moeny; 
        }elseif($utype=='agencys' && !empty($user1) && !empty($uid)){
             ...
             if($utype=='agencys'  && !empty($uid)){
              ...
              //dump($where1);exit;
               ...
               // $data2['contents']='修改代理'. $uname.',信用积分'.$data['au_moeny'];
                ...
                $arr=array('code'=>true,'titles'=>'修改成功','urls'=>'/User/index');
                $stats=json_encode($arr);   
            }else{
                $arr=array('code'=>true,'titles'=>'修改成功','urls'=>'/User/index');
                $stats=json_encode($arr);
	    ...
	    $data['au_session'] = '';
            ...
            if(!empty($user)){//上级
               if($utype=='agencys'  && empty($uid)){
                ...
                $arr=array('code'=>true,'titles'=>'添加成功','urls'=>'/User/index');
                $stats=json_encode($arr);   
            }else{
                $arr=array('code'=>true,'titles'=>'添加失败','urls'=>'/User/index');
                $stats=json_encode($arr);
            }
        }
    ...
    //超管(admin),股东(partner),总代理(agencys),代理(agency),测试(demo)用户类型
        $admins=M('admin');
       if(session('autype')=='agency'){//代理--->会员
    ...
    public function saveUser1(){
        ...
        if(empty($uid) && !empty($user1)){
          ...
          if($integral>$money1){
            $arr=array('code'=>false,'titles'=>'积分大于账号积分！','urls'=>'/User/user1');
            $stats=json_encode($arr);
            echo $stats;    
            exit;
          }
          $data['money']=$integral; 
        }elseif(!empty($user1) && !empty($uid)){
          $where['uid']=$uid;//用户ID
          ...
          $arr=array('code'=>false,'titles'=>'参数不齐，操作不成功','urls'=>'/User/user1');
          $stats=json_encode($arr);
          echo $stats;    
          exit;
        }
        ...
        if(!empty($uid)){
         if(session('autype')=='agency'){
          ...
          if(!empty($user)){
              ...
              $arr=array('code'=>true,'titles'=>'修改成功','urls'=>'/User/user1');
              $stats=json_encode($arr);   
          }else{
              $arr=array('code'=>false,'titles'=>'修改失败','urls'=>'/User/user1');
              $stats=json_encode($arr);
          }
            ...
            if(!empty($id)){
                ...
                $arr=array('code'=>true,'titles'=>'添加成功','urls'=>'/User/user1');
                $stats=json_encode($arr);   
            }else{
                $arr=array('code'=>false,'titles'=>'添加失败','urls'=>'/User/user1');
                $stats=json_encode($arr);
            }
    ...
    public function findUser(){
        ...
        if(empty($user)){ 
            $arr=array('code'=>false);
            $stats=json_encode($arr);
        }elseif($user['au_id']==$uid){
            $arr=array('code'=>false);
            $stats=json_encode($arr);
        }else{
            $arr=array('code'=>true);
    ...
    //随机码
    private function generate_password( $length = 6 ) {// 密码字符集，可任意添加你需要的字符     
...
public function delsUser(){
    ...
    if($utype=='admin'){
      ...
      if(!empty($uid)){
        ...
        //     }
        // }
        $where1['top_uid']=array('in',rtrim($uid,','));//得到所有总代理
        ...
        $where2['top_uid']=array('in',rtrim($uid3,','));//得到所有代理
        ...
        if(!empty($data2)){
          ...
          $where4['uid']=array('in',rtrim($uid5,','));//得到所有代理
      ...
      $where4['au_id']=array('in',rtrim($uidarr,','));//得到所有代理
      ...
      //   $data1=$admins->field('au_id')->where($where)->select();
        if(!empty($uid)){
          ...
          //     }
          // }
        $where2['top_uid']=array('in',rtrim($uid,','));//得到所有代理
        $data2=$admins->field('au_id')->where($where2)->select();
        if(!empty($data2)){
          foreach($data2 as $k2=>$v2){
              if($v2['au_id']){
      ...
      //   $data1=$admins->field('au_id')->where($where)->select();
        if(!empty($uid)){
          ...
          //     }
          // }
           $where3['top_uid']=array('in',rtrim($uid,','));//得到所有代理
           $data3=M('user')->where($where3)->select();
          if(!empty($data3)){
      ...
      //$uidarr=$uid;
      $where5['au_id']=array('in',rtrim($uid,','));//得到所有代理
      ...
      // $dd=M('user')->where($where3)->delete();
      $where4['uid']=array('in',rtrim($uid5,','));//得到会员
      $dd=M('bet')->where($where4)->delete();
    }
    $arr['code']=200;
    $this->ajaxReturn($arr);
    ...
    if($types=='admin'){//股东下所有账号
      ...
      if($data2){//股东下所有的代理
        ...
        if($data3){//股东下所有的代理
          ...
          //   }
            //所有会员
            $where5['top_uid']=array('in',rtrim($auid1,','));
            $data5['status']=$status;
      ...
      $id=$auid.$auid1;
      //dump($id);exit;
      $where4['au_id']=array('in',$id);
      
      $dd=$admin->where($where4)->save($data);
      
      
    }elseif($types=='partner'){//总代理下所有账号
      $where1['top_uid']=$auid11;
      ...
      if($data2){//股东下所有的代理
        ...
        //   }
          //所有会员
          $where5['top_uid']=array('in',rtrim($auid,','));
          $data5['status']=$status;
          M('user')->where($where5)->save($data5);
        //}
      }
...
public function saveMoneys(){
    ...
    if(empty($money) || empty($uid)){
      $arr=array('code'=>false,'titles'=>'错误操作！','urls'=>'/User/index');
      ...
      if($sum){//操作日志
          ...
          $arr=array('code'=>false,'titles'=>'添加失败！','urls'=>'/User/index');
      }

    }elseif($utype=='agencys' || $utype=='agency'){
      $where1['au_id']= $auid;
      $where1['au_type']= $utype;
      ...
      if(empty($money) ||  $moneys>($data['au_money']-$data['au_moneys'])){
        $arr=array('code'=>false,'titles'=>'没有填写信用额度/或者大于你的账号信用积分！','urls'=>'/User/index');
      }else{
            if($counts){
                for($a=0;$a<count($uid1);$a++){
                    if($uid1[$a]){
...
public function delsMoenys(){
  ...
  $admins=M('admin');
  if(empty($uid)){
    $arr=array('code'=>false,'titles'=>'操作失败！','urls'=>'/User/index');
  }else{
    $uid1=explode(',',$uid);
    $counts=count($uid1);
    if($counts){
      ...
      if($sum){//操作日志
          ...
          $arr=array('code'=>true,'titles'=>'操作成功！','urls'=>'/User/index');
      }else{
          $arr=array('code'=>false,'titles'=>'操作失败！','urls'=>'/User/index');
      }
    }else{
...
public function saveMoneys1(){
    ...
    $data=$admins->field('au_money,au_moneys')->where($where1)->find();
    if(empty($money) || empty($uid)){
      $arr=array('code'=>false,'titles'=>'错误操作！','urls'=>'/User/user1');
    }else{
      $users=M('user');
      $uid1=explode(',',$uid);
      $counts=count($uid1);
      ...
      if($moneys>($data['au_money']-$data['au_moneys'])){
        $arr=array('code'=>false,'titles'=>'填写信用额度大于你的账号信用积分！','urls'=>'/User/user1');
      }else{
         if($counts){
                for($a=0;$a<count($uid1);$a++){
                    if($uid1[$a]){
  ...
  public function delsMoenys1(){
    ...
    $users=M('user');
    if(empty($uid)){
      $arr=array('code'=>false,'titles'=>'操作失败！','urls'=>'/User/user1');
    }else{
      $uid1=explode(',',$uid);
      $counts=count($uid1);
      $admins=M('admin');
      if($counts){
        ...
        if($sum){//操作日志
            ...
            $arr=array('code'=>true,'titles'=>'操作成功！','urls'=>'/User/user1');
        }else{
            $arr=array('code'=>false,'titles'=>'操作失败！','urls'=>'/User/user1');
        }
      }else{
    ...
    public function saveAdmins(){
        ...
        if(!empty($password)){
            ...
            $data1=M('admin')->where($where)->save($data); 
             if($data1){
                $arr=array('code'=>true,'titles'=>'修改成功！','urls'=>'/Login/logout');
            }else{
                $arr=array('code'=>false,'titles'=>'修改失败，没有改动的内容！','urls'=>'/Login/logout');
            }
