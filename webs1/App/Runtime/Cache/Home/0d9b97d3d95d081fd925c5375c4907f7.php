...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 总货明细 <span class="c-gray en">&gt;</span> 中奖明细 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
...
<form action="/Data/download" method="post">
  ...
  function findData(){
    ...
    window.location.href="/Data/lotteryOrder?qishu="+qishu+'&uname='+uname+'&number='+number+'&xian='+xian+'&condition='+condition+'&start='+start+'&finish='+finish+'&category='+category+'&types='+types;
  }
  </script>
   <div class="mt-20">
    ...
    <table class="table table-border table-bordered table-bg table-hover table-sort">
      ...
      <?php if($data): ?><!--  <tr class="bg4 text-c" title="已退码" >  <td>  283258761131322712  </td> <td class="bg3"> xiaohui </td> <td>  11-24 20:17:56 退 11-24 20:18:00  </td> <td class="bg3 fb">0199</td> <td>20</td> <td class="bg3">8800</td>  <td>--</td>  <td class="bg3">1.6</td> <td>18.4</td>   </tr> -->
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["js"] == '2'): ?><!-- <tr class="text-c"   style="text-decoration:line-through;"> -->
          ...
          <?php if($vo["status"] == '1'): ?><td><?php echo ($vo['yingkui']); ?></td>
            <?php else: ?>
               <td><?php echo ($vo['money']-$vo['win']); ?></td><?php endif; ?>
          <!--  <td class="bg3">
            <?php if($vo["status"] == '1'): echo ($vo['moneys']); endif; ?>
          </td>
          <td>
            <?php if($vo["status"] == '1'): echo ($vo['yingkui']); endif; ?>
          </td> -->
         ...
         <tr>
          ...
          <td><?php echo ($data1['money']-$data1['moneys']-$data1['win']); ?></td>
        </tr>
      <?php else: ?>
...
<script>
  ...
  $('#pages').click(function(){
    ...
    var finish=$('input[name="finish"]').val();
    var category=$('select[name="category"] option:selected').val();
    window.location.href="/Data/lotteryOrder?uname="+uname+"&number="+number+"&xian="+xian+"&condition="+condition+"&start="+start+"&finish="+finish+"&category="+category+"&qishu="+qishu+"&p="+page1;
  });
